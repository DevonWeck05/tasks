...
export function getPublishedQuestions(questions: Question[]): Question[] {
    return questions.filter((p: Question): boolean => p.published);
}
...
export function findQuestion(
    ...
    let index: number = questions
        .map((q: Question): number => q.id)
        .findIndex((i: number): boolean => i === id);
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (totalP: number, q: Question) => totalP + q.points,
        0,
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return sumPoints(questions.filter((q: Question): boolean => q.published));
}
...
export function sameType(questions: Question[]): boolean {
    ...
    let first: string = questions[0].type;
    return questions.every((q: Question): boolean => q.type === first);
}
...
export function duplicateQuestionInArray(
    ...
    let duped: Question = duplicateQuestion(newId, questions[index]);
    return [
